...
// Console.WriteLine(newText);
// --------------------------------------------------------------------------------------
// Напишу алгоритм сортировки массива методом выбора.

int [] arr = {1, 5, 4 ,3 ,2 ,6 ,7 ,1 ,1 }; // Определю массив.

// Опишу метод, который будет решать подзадачу и выводить данный массив на экран.
// Первый способ.
void PrintArray(int[] array) 
{
    int count = array.Length;
    for (int i = 0; i < count; i++) // В этом цикле пробугаю по всем элементам массива.
    {
        Console.Write($"{array[i]} "); // Показываю на экране в одну строку через пробел.
    }
    Console.WriteLine(); // Выведу пустую строку, для разделения.
}

// Следующим этапом опишу метод, который будет упорядочивать массив от min до max.
void SelectionSort(int[] array) // Опишу новый метод
{
    for (int i = 0; i < array.Length - 1; i++) // 1(4!). Нужно пробужать по всему массиву.
    {
         int minPosition = i; // 2. Определю позицию на которую смотрю и запоминаю позицию 
        //  рабочего элемента, для которого дальше буду производить действия.

        for (int j = i + 1; j < array.Length; j++) // 4. Ищу минимальный элемент. Первый кусочек массива, 
        // который уже был отсортирован, я его не косаюсь, поэтому начинаю с (i + 1).
        // Важно!!! Если я начинаю поиск позиции поиска min от i + 1, где i меняется до максимального 
        // значения позиции массива, то в этом случае, я должен искуственно отнять 1 (array.Length - 1).
        // Тогда i + 1 даст общее количество элементов.
        {
            if(array[j] < array[minPosition]) minPosition = j; // 5. Ищу минимальный элемент. Смотрю 
            // текущий, если он меньше минимальной позиции, то нужно будет эту позицию сохранить.
        }  
        
        // 3. После выполнения части кода выше мне нужно поменять значения минимальной с той позицией, 
        // которую я нашел. Это простой обмен двух переменных местами.
